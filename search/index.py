...
class InvertedIndexReader(InvertedIndex):
    ...
    def get_postings_list(self, term):
        ...
        """
        start, _, byte_postings, byte_tf = self.postings_dict[term]

        # Ubah posisi pointer
        self.index_file.seek(start)

        # Membaca dari file lalu decode
        postings_list = self.postings_encoding.decode(self.index_file.read(byte_postings))
        tf_list = self.postings_encoding.decode_tf(self.index_file.read(byte_tf))
    ...
    def get_average_document_length(self):
        ...
        """
        count_docs = len(self.doc_length)
        total_length = 0
        for length in self.doc_length.values():
            total_length += length
        return total_length/count_docs
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        for i in range(len(postings_list)):
            # Update doc_length berdasarkan tf_list
            self.doc_length[postings_list[i]] = self.doc_length.get(postings_list[i], 0)\
                  + tf_list[i]
        
        # Encode postings list dan tf list untuk dimasukkan ke index
        postings_encoded = self.postings_encoding.encode(postings_list)
        tf_encoded = self.postings_encoding.encode_tf(tf_list)

        # Ambil current pointer lalu isi ke postings dict
        current_pointer = self.index_file.tell()
        self.postings_dict[term] = (current_pointer, len(postings_list), 
                                    len(postings_encoded), len(tf_encoded))
        
        # Tulis ke index
        self.index_file.write(postings_encoded)
        self.index_file.write(tf_encoded)
